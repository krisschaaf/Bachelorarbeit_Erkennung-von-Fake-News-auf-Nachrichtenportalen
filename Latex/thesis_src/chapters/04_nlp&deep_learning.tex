...
Word Embeddings verstehen die Bedeutung der einzelnen Wörter im gesamten Kontext \cite{Deshai:2023aa}
...
Word Embeddings erkennt diese und erstellt für die zwei Kontexte/Wörter zwei verschiedene Vektoren \cite{skopos2023wordembeddings}.
...
Diese werden in einer globalen Co-Occurrence-Matrix zusammengefasst, die darstellt, wie oft Wörter zusammen im Korpus vorkommen 
\cite{Wang:2020aa}.

Nach Erstellung der Matrix wird eine Zielfunktion formuliert, die die Beziehung zwischen den Wortvektoren und ihren 
Koinzidenzwahrscheinlichkeiten beschreibt .

Die Zielfunktion wird mithilfe von Gradientenabstieg oder anderen Optimierungsalgorithmen optimiert. Das Ziel ist es, 
die Wortvektoren und Verzerrungen so anzupassen, dass die quadratische Differenz zwischen der vorhergesagten und der 
tatsächlichen logarithmischen Wahrscheinlichkeit des gemeinsamen Auftretens minimiert wird \cite{ibm2024wordembeddings}.



Gegeben ein Korpus mit $U$ Wörtern, dann ist die Kookkurrenzmatrix $X \in \mathbb{R}^{U \times U}$.  
Das Element $X_{ij}$ steht für die Häufigkeit, mit der das Wort $w_j$ im Kontext von $w_i$ auftritt.
...
\subsection*{2. Verhältnis von Wahrscheinlichkeiten}
...
\subsection*{3. Vektorielle Darstellung}
...
Dabei sind:
\begin{itemize}
  \item $\vec{w}_i, \vec{\tilde{w}}_j$ die Vektoren (Embeddings) der Wörter $i$ und $j$,
  \item $b_i, \tilde{b}_j$ Bias-Terme.
\end{itemize}

\subsection*{4. Gewichtete Kostenfunktion}

Um seltene Wortpaare weniger stark zu gewichten, wird eine Gewichtungsfunktion $f(X_{ij})$ verwendet:

\[
J = \sum_{i,j=1}^U f(X_{ij}) \left( \vec{w}_i^T \vec{\tilde{w}}_j + b_i + \tilde{b}_j - \log(X_{ij}) \right)^2
\]

Typisch ist:
\[
f(x) = \begin{cases}
(x/x_{\text{max}})^\alpha & \text{wenn } x < x_{\text{max}} \\
1 & \text{sonst}
\end{cases}
\quad \text{mit } \alpha = 0.75
\]

\subsection*{5. Vorteile von GloVe}

\begin{itemize}
  \item Nutzt globale Kookkurrenzstatistik (anders als Word2Vec).
  \item Robust gegenüber seltenen Wörtern.
  \item Gut für Analogieaufgaben geeignet.
\end{itemize}
